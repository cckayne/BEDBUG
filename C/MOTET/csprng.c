...
char* rName(enum CSPRNG rng) {
	if (rng==MOTE8)  return mote8_Name();  else
	if (rng==MOTE16) return mote16_Name(); else
	if (rng==MOTE32) return mote32_Name(); else
	if (rng==BB512)  return "BB512";  else
	if (rng==BB128)  return "BB128";  else
	if (rng==BB256)  return "BB256";  else
	if (rng==ISAAC)  return "ISAAC";  else
	return "";
}